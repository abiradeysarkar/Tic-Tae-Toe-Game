...
class Database:
    ...
    def doQuery(self, conn):
        cur = conn.cursor()
        cur.execute(f"SELECT player_1,player_2,score FROM {database}.{table}")
    ...
    def insert(self, conn, history: History):
        cur = conn.cursor()
        cur.execute(
            f"""insert into {database}.{table} (player_1,player_2,score) values('{history.player1}', '{history.player2}', {history.winner});""")
        cur.execute("commit;")
        for player_1, player_2, score in cur.fetchall():
            print(player_1, player_2, score)
